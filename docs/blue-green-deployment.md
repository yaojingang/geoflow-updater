...
恢复服务之前，updater 先持久记录可能重新接受业务写入的边界。恢复服务或验收失败后，有新版明确边界记录的操作会自动重试服务恢复和验收；再次覆盖数据需要发起新的 `rollback --recovery-point` 操作。旧操作即使已有 resume/verify 记录，也可能曾再次回灌并中途崩溃，实例保留 `recovery_required`，由主机管理员核对数据及恢复点后启动明确恢复。此规则同时覆盖手工恢复、维护升级的失败恢复和蓝绿源版本恢复。在线应用切回继续保留当前数据。
