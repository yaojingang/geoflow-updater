...
10. Store the completed JSON as `PHASE_C_REHEARSAL_EVIDENCE_B64` in the protected environment. If version-tag creation is restricted to a designated maintainer, that identity must create and verify the signed updater tag at the exact candidate source commit before dispatching `release.yml`; keep the tag protection active. Dispatch with the candidate run ID and decoded evidence SHA-256. The gate rejects missing required cases, mixed candidate runs, emulated architectures, incomplete host modes, and missing approvals.
11. Confirm the updater commit, GEOFlow commit, target bytes, image digests, and archive attestations still match the candidate. First publication accepts the exact updater candidate commit or a descendant whose intervening commits only modify `timestamp.json` and add immutable numbered snapshot files. The gate checks every parent edge, including changes later reverted; all target references, root metadata and existing target bytes stay fixed. It verifies the current TUF signature chain, target bytes and expiry through the existing repository verifier. Source, workflow, dependency, Compose, root or targets changes require a new candidate and fresh acceptance. Harness-only descendants may debug an earlier candidate, but cannot substitute for final publication evidence.
12. Confirm the GitHub Release is public before Pages serves the new TUF timestamp. The publisher promotes the tested main candidate image digests and publishes `candidate/targets-source`. The online fixture remains outside that path. The release workflow dispatches Pages asynchronously; wait for that Pages run to succeed and read back the public timestamp and its referenced targets before marking publication complete. It publishes the updater with `--latest=false`; promote the intended stable release to Latest only after the complete version pair and public update metadata are available, then verify the Latest bootstrap manifest.
...
- The candidate workflow builds images under run-specific candidate tags, signs an isolated TUF repository, and uploads an immutable artifact. It does not change public TUF metadata, public releases, version tags, or Pages. Candidate artifacts are eligible for at most 30 days from creation and only while GitHub still retains the exact original artifact. Publication checks the original run, source, artifact identity and expiry. Missing or expired candidates require a new dispatch and fresh acceptance; rerunning a candidate workflow is rejected because it would reuse the old run ID. An unchanged version number cannot reuse approval for a rebuilt candidate.
...
- First publication after daily refresh and continuation after a committed publication use separate source gates. A first-publication attempt cannot introduce new targets metadata or target bytes; the continuation path below requires the committed candidate target set. Manual publication and scheduled metadata refresh share the `geoflow-release` concurrency group.
- Read-only preflight determines resume eligibility from committed metadata. Draft releases require push access to read, so the existing-release check runs inside the protected publish job with its existing write permission. A missing or inaccessible draft still stops publication before candidate promotion; check the draft and credentials without broadening preflight permissions.
- If the metadata commit succeeds and a later publication step fails, dispatch a new `release.yml` run from current `main` with the same candidate, versions, sequence and approved evidence. The resume gate accepts an ancestor candidate only when subsequent changes consist of a timestamp update and newly added immutable snapshot, targets metadata or target files. Source, workflows, dependencies, installer, Compose and trust roots remain identical; deletion, replacement, renaming and symlinks are rejected. It verifies the current TUF signature chain and expiry, and requires the complete signed target set and published bytes to match `candidate/targets-source`. Archive attestations remain bound to the original candidate commit. The resumed run validates release assets, promotes matching image digests and dispatches Pages without signing a second release. Rerunning an older job keeps its original checkout; use a new dispatch so preflight can observe the committed publication metadata.
