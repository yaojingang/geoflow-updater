...
11. Confirm the updater commit, GEOFlow commit, target bytes, image digests, and archive attestations still match the candidate. The first publication requires the same updater `main` commit used to build the candidate. Merge runtime fixes before building a replacement candidate and rerun acceptance. Harness-only descendants may debug an earlier candidate, but cannot substitute for final publication evidence.
